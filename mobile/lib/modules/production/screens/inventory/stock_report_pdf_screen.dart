...
enum _StockReportPdfPhase { preparing, viewing, error }

/// Downloads Inventory Stock Report PDF with Bearer auth, validates bytes, then
/// opens a local [PDFView] (same pattern as Item Stock Ledger PDF).
class StockReportPdfScreen extends StatefulWidget {
  const StockReportPdfScreen({
    super.key,
    required this.api,
    this.inventoryType,
    this.itemKey,
  ...
  @override
  State<StockReportPdfScreen> createState() => _StockReportPdfScreenState();
}

class _StockReportPdfScreenState extends State<StockReportPdfScreen> {
  static const _logName = 'InventoryStockReportPdf';

  _StockReportPdfPhase _phase = _StockReportPdfPhase.preparing;
  String? _errorMessage;
  String? _localPath;
  String _filename = 'Inventory_Stock_Report.pdf';
  ...
  Future<void> _preparePdf() async {
    _loadWatchdog?.cancel();
    setState(() {
      _phase = _StockReportPdfPhase.preparing;
      _errorMessage = null;
    ...
    try {
      final result = await widget.api.downloadStockReportPdf(
        inventoryType: widget.inventoryType,
        itemKey: widget.itemKey,
        search: widget.search,
        stockStatusFilter: widget.stockStatusFilter,
      );
      ...
      final file = File('${dir.path}/stock_report_${stamp}_$safeName');
      await file.writeAsBytes(bytes, flush: true);

      if (!await file.exists()) {
        throw StateError('Failed to write stock report PDF to app temp storage.');
      }
      ...
      setState(() {
        ...
        _filename = safeName;
        _phase = _StockReportPdfPhase.viewing;
      });
      ...
      _loadWatchdog = Timer(const Duration(seconds: 20), () {
        ...
        setState(() {
          _phase = _StockReportPdfPhase.error;
          _errorMessage =
              'PDF viewer timed out while loading the stock report. Please retry.';
        });
      });
    } catch (e, st) {
  ...
  String _friendlyError(Object e) {
    ...
    return raw.isEmpty
        ? 'Unable to prepare Inventory Stock Report PDF.'
        : raw;
  }
  ...
  String _safeFilename(String name) {
    ...
    final type = (widget.inventoryType ?? 'All')
    ...
    final stamp =
        ...
        '${now.year}';
    return 'Inventory_Stock_Report_${type}_$stamp.pdf';
  }
  ...
  void _onRender(int? pages) {
    ...
    if (pages == null || pages <= 0) {
      setState(() {
        ...
        _phase = _StockReportPdfPhase.error;
        _errorMessage =
            'Failed to open Inventory Stock Report PDF in viewer.';
      });
      return;
    }
  ...
  Widget build(BuildContext context) {
    final canShare = _phase == _StockReportPdfPhase.viewing &&
        _documentReady &&
        _localPath != null;
    ...
    return Scaffold(
      ...
      body: switch (_phase) {
        _StockReportPdfPhase.preparing => const _PreparingBody(),
        _StockReportPdfPhase.error => _ErrorBody(
            message: _errorMessage ??
                'Unable to prepare Inventory Stock Report PDF.',
            onRetry: _preparePdf,
            onBack: () => Navigator.of(context).maybePop(),
          ),
        _StockReportPdfPhase.viewing => _buildViewer(),
      },
    );
  }
  ...
  Widget _buildViewer() {
    ...
    if (path == null) {
      return _ErrorBody(
        message: 'Stock report PDF is missing from local storage.',
        onRetry: _preparePdf,
        onBack: () => Navigator.of(context).maybePop(),
      );
    }
      ...
      children: [
        ...
        if (!_documentReady)
          const ColoredBox(
            ...
            child: _PreparingBody(label: 'Opening Inventory Stock Report…'),
          ),
      ],
    );
  }
...
class _PreparingBody extends StatelessWidget {
  const _PreparingBody({this.label = 'Preparing Inventory Stock Report…'});
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                OutlinedButton(onPressed: onBack, child: const Text('Cancel')),
                FilledButton.tonal(
                  onPressed: onRetry,
