...
class RawMaterialInwardInfolist
{
                ...
                Section::make('Inward Summary')
                    ...
                    ->schema([
                        TextEntry::make('total_materials')
                            ...
                            ->state(fn (RawMaterialInward $record): int => $record->items->count()),
                    ...
                    ->schema([
                        RepeatableEntry::make('items')
                            ->schema([
                                ...
                                TextEntry::make('material_name')->label('Raw Material'),
                                TextEntry::make('stock_before')
                                    ->label('Current Stock Before Inward')
                                    ->numeric(3)
                                    ->state(function ($record): float {
                                        if ($record->stock_before !== null) {
                                            return (float) $record->stock_before;
                                        }

                                        return (float) ($record->rawMaterial?->current_stock ?? 0);
                                    }),
                                TextEntry::make('old_average_rate')
                                    ...
                                    ->state(function ($record): float {
                                        if ($record->old_average_rate !== null) {
                                            return (float) $record->old_average_rate;
                                        }

                                        return (float) ($record->rawMaterial?->average_rate ?? 0);
                                    }),
                                ...
                                TextEntry::make('stock_after')
                                    ...
                                    ->numeric(3)
                                    ->state(function ($record): float {
                                        if ($record->stock_after !== null) {
                                            return (float) $record->stock_after;
                                        }

                                        $before = (float) ($record->stock_before ?? $record->rawMaterial?->current_stock ?? 0);
                                ...
                                TextEntry::make('new_average_rate')
                                    ...
                                    ->money('INR')
                                    ->visible($canViewRates)
                                    ->state(function ($record) use ($canViewRates): ?float {
                                        if (! $canViewRates) {
                                            return null;
                                        }
                                        if ($record->new_average_rate !== null) {
                                            return (float) $record->new_average_rate;
                                        }

                                        $oldStock = (float) ($record->stock_before ?? $record->rawMaterial?->current_stock ?? 0);
                                        $oldAvg = (float) ($record->old_average_rate ?? $record->rawMaterial?->average_rate ?? 0);
