...
class RawMaterialInwardApiController extends Controller
{
    use RespondsWithJson;

    public function __construct(
        private readonly RawMaterialInwardService $service,
    ) {}

    public function index(Request $request): JsonResponse
    {
        $this->authorize('viewAny', RawMaterialInward::class);
        ...
        if (! empty($validated['raw_material_id'])) {
            $query->whereHas('items', fn ($q) => $q->where('raw_material_id', $validated['raw_material_id']));
        }
        ...
        $user = $request->user();
        $rows = $query->limit(100)->get()->map(fn (RawMaterialInward $inward) => $this->presentList($inward, $user));

        return $this->ok('Raw material inwards loaded.', ['items' => $rows]);
    }

    public function store(Request $request): JsonResponse
    {
        // Mobile production API: PS may not create/edit inwards (Filament web unchanged).
        abort(403, 'Raw material inward create is not available via the mobile API.');
    }

    public function show(Request $request, RawMaterialInward $inward): JsonResponse
    {
        $this->authorize('view', $inward);

        $inward->load(['items.rawMaterial', 'supplier', 'createdBy', 'batches']);

        return $this->ok('Inward details loaded.', $this->presentDetail($inward, $request->user()));
    }

    public function update(Request $request, RawMaterialInward $inward): JsonResponse
    {
        abort(403, 'Raw material inward update is not available via the mobile API.');
    }

    public function post(Request $request, RawMaterialInward $inward): JsonResponse
    {
        abort(403, 'Raw material inward post is not available via the mobile API.');
    }

    public function cancel(Request $request, RawMaterialInward $inward): JsonResponse
    {
        abort(403, 'Raw material inward cancel is not available via the mobile API.');
    }
    ...
    public function searchRawMaterials(Request $request): JsonResponse
    {
        ...
        $items = RawMaterial::query()
            ...
            ->map(fn (RawMaterial $m) => [
                ...
                'batch_tracking_enabled' => (bool) $m->batch_tracking_enabled,
                'expiry_tracking_enabled' => (bool) $m->expiry_tracking_enabled,
                'purchase_rate' => $request->user()->canViewInwardRates() ? (float) $m->purchase_rate : null,
            ]);

        return $this->ok('Raw materials loaded.', ['items' => $items]);
    }
    ...
    public function batchDetails(Request $request, RawMaterialBatch $batch): JsonResponse
    {
        $this->authorize('viewAny', RawMaterialInward::class);

        $batch->load('rawMaterial');

        return $this->ok('Batch details loaded.', [
            'id' => $batch->id,
            ...
            'material_name' => $batch->rawMaterial?->material_name,
            'expiry_date' => optional($batch->expiry_date)?->toDateString(),
            'available_quantity' => (float) $batch->available_quantity,
            'effective_unit_rate' => $request->user()->canViewInwardRates()
                ? (float) $batch->effective_unit_rate
                : null,
     ...
     */
    private function validateInwardPayload(Request $request): array
    {
        $validated = $request->validate([
     ...
     */
    private function presentList(RawMaterialInward $inward, $user): array
    {
        return [
            'id' => $inward->id,
     ...
     * @return array<string, mixed>
     */
    private function presentDetail(RawMaterialInward $inward, $user): array
    {
        ...
        return [
            ...
            'items' => $inward->items->map(function ($item) use ($canRates) {
                $stockBefore = $item->stock_before !== null
                    ...
                    : (float) ($item->rawMaterial?->current_stock ?? 0);
                $oldAvg = $item->old_average_rate !== null
                    ? (float) $item->old_average_rate
                    : (float) ($item->rawMaterial?->average_rate ?? 0);
