...
/**
 * Creates a semi-finished material master with optional opening stock.
 *
 * Opening stock updates semi-finished inventory + average production cost and
 * writes an Opening Stock ledger entry only (StockItemType::SemiFinished).
 * It does NOT create Production Entry or Material Inward records, and never
 * posts into raw material, packaging, or finished goods stock.
 */
final class SemiFinishedMaterialCreateService
{
     ...
     * }  $opening
     */
    public function create(array $materialData, array $opening, User $user): SemiFinishedMaterial
    {
        return DB::transaction(function () use ($materialData, $opening, $user): SemiFinishedMaterial {
    ...
    private function applyOpeningStock(SemiFinishedMaterial $material, array $opening, User $user): void
    {
        ...
        $lockedMaterial = $this->inventoryService->lockSemiFinishedMaterial($material->id);
        $oldStock = (float) $lockedMaterial->current_stock;
        $oldAvg = (float) $lockedMaterial->average_production_cost;
        ...
        $this->ledgerService->postSemiFinishedMovement(
            ...
            [
                ...
                'reference_type' => SemiFinishedMaterial::class,
                ...
                'remarks' => $remarks !== '' ? $remarks : 'Semi-Finished Material Creation',
            ],
            $user,
        );
        ...
        $lockedMaterial->average_production_cost = $newAvg;
        $lockedMaterial->current_stock_value = round((float) $lockedMaterial->current_stock * $newAvg, 2);
        $lockedMaterial->save();
    }

    /**
     * Resolve Effective Rate via MaterialInwardCosting (same path as raw / packaging masters).
     *
     * @param  array<string, mixed>  $opening
