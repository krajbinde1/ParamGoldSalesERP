    ...
    <div class="space-y-6">
        <div class="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
            ...
            <ul class="mt-4 list-disc space-y-1 pl-5 text-sm text-gray-600 dark:text-gray-300">
                <li>Template is generated from Sales Operations → Products and pre-fills Product Code + Product Name.</li>
                <li>Fill only Opening Stock Quantity, Opening Stock Value, and Opening Stock Date.</li>
                <li>Blank rows are ignored. Matching uses Product Code only (Product Name is for reference/validation).</li>
                <li>Does not create, delete, or recreate products — and does not create production or material inward.</li>
            </ul>
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">
                            Nothing is saved yet. Review each filled row, then confirm to import valid rows only.
                        </p>
                    </div>
                ...
                @if ($previewCounts !== null)
                    ...
                    @if (($previewCounts['ignored'] ?? 0) > 0)
                        <p class="mt-3 text-xs text-gray-500 dark:text-gray-400">
                            {{ $previewCounts['ignored'] }} blank template row(s) were ignored.
                        </p>
                    @endif
                @endif
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-900">
                            <tr class="text-left">
                                ...
                                <th class="px-4 py-3">Opening Stock Quantity</th>
                                <th class="px-4 py-3">Opening Stock Value</th>
                                <th class="px-4 py-3">Opening Stock Date</th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            @forelse ($previewRows as $row)
                                ...
                                <tr>
                                    ...
                                    <td class="px-4 py-3 font-medium">{{ $data['product_code'] ?? '-' }}</td>
                                    <td class="px-4 py-3">{{ $data['product_name'] ?? '-' }}</td>
                                    <td class="px-4 py-3">{{ $data['opening_quantity'] ?? '-' }}</td>
                                    <td class="px-4 py-3">{{ $data['opening_value'] ?? '-' }}</td>
                                    <td class="px-4 py-3">{{ $data['opening_date'] ?? '-' }}</td>
                                    <td class="px-4 py-3">
                                        @if ($row['is_valid'])
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-4 py-8 text-center text-gray-500">
                                        No filled opening-stock rows found. Enter Quantity, Value, and Date for at least one Finished Product.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                @if (($previewCounts['total'] ?? 0) > count($previewRows))
                    <p class="mt-3 text-xs text-gray-500 dark:text-gray-400">
                        Showing a sample of {{ count($previewRows) }} rows. All filled rows were validated and will be processed on confirm.
                    </p>
                @endif
            </div>
        @endif
        ...
        @if ($phase === 'result' && $summary !== null)
            <div class="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
                ...
                <div class="mt-6 grid gap-4 sm:grid-cols-3">
                    <div class="rounded-xl border border-gray-200 p-4 dark:border-gray-700">
                        <div class="text-sm text-gray-500 dark:text-gray-400">Rows Processed</div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-900">
                                <tr class="text-left">
                                    ...
                                    <th class="px-4 py-3">Opening Rate</th>
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                @foreach ($importedRows as $row)
                                    <tr>
                                        ...
                                        <td class="px-4 py-3">{{ $row['opening_rate'] ?? '-' }}</td>
                                        <td class="px-4 py-3">
                                            <span class="inline-flex rounded-full bg-success-50 px-2.5 py-1 text-xs font-medium text-success-700 dark:bg-success-500/10 dark:text-success-400">
                                                {{ $row['status'] ?? 'Success' }}
                                            </span>
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                @foreach (array_slice($failedRows, 0, 100) as $error)
                                    <tr>
                                        ...
                                        <td class="px-4 py-3">{{ $error['data']['product_code'] ?? '-' }}</td>
