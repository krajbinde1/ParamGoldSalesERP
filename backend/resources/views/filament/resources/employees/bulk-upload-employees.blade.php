        ...
        @if ($step === 1)
            <div class="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
                <div class="max-w-2xl space-y-4">
                    <div>
                        <h2 class="text-lg font-semibold text-gray-950 dark:text-white">Upload Employee File</h2>
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">
                            Use the employee Excel template. Employee Code and Login ID are generated automatically from the mobile number. Existing Mobile, Email, Aadhaar, or PAN values will update the matching employee.
                        </p>
                    </div>
        ...
        @if ($step === 2)
            <div class="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <h2 class="text-lg font-semibold text-gray-950 dark:text-white">Preview Uploaded Employees</h2>
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">
                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        >
                            <span wire:loading.remove wire:target="runImport">Import Employees</span>
                            <span wire:loading wire:target="runImport">Importing employees...</span>
                        </x-filament::button>
                    </div>
                </div>
                ...
                <div wire:loading wire:target="runImport" class="mt-4 rounded-lg border border-primary-200 bg-primary-50 px-4 py-3 text-sm text-primary-700 dark:border-primary-500/30 dark:bg-primary-950 dark:text-primary-300">
                    Import in progress. Please wait while employees are created or updated.
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-900">
                            <tr class="text-left">
                                ...
                                <th class="px-4 py-3">Manager Code</th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            @forelse ($previewRows as $row)
                                <tr>
                                    ...
                                    <td class="px-4 py-3">{{ $row['data']['full_name'] ?? '-' }}</td>
                                    <td class="px-4 py-3">{{ $row['data']['mobile'] ?? '-' }}</td>
                                    <td class="px-4 py-3">{{ filled($row['data']['email'] ?? null) ? $row['data']['email'] : '-' }}</td>
                                    <td class="px-4 py-3">{{ $row['data']['role'] ?? '-' }}</td>
                                    <td class="px-4 py-3">{{ filled($row['data']['reporting_manager_employee_code'] ?? null) ? $row['data']['reporting_manager_employee_code'] : '-' }}</td>
                                    <td class="px-4 py-3">{{ $row['data']['aadhaar_number'] ?? '-' }}</td>
                                    <td class="px-4 py-3">{{ $row['data']['pan_number'] ?? '-' }}</td>
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">
                            Bulk employee upload completed. Valid rows were imported and invalid rows were skipped.
                        </p>
                    </div>
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                @foreach ($failedRows as $error)
                                    <tr>
                                        ...
                                        <td class="px-4 py-3">{{ $error->rowData['full_name'] ?? '-' }}</td>
                                        <td class="px-4 py-3">{{ $error->rowData['mobile'] ?? '-' }}</td>
